...
class RepeatedStratifiedNestedCV:
    ...
    cv_options : dict, default={}
        ...
        'threshold_tuning_scoring' : str or list, default='f2'
            Only used, if ``cv_options['tune_threshold']=True``.
            If a single metric is chosen as scoring (e.g. ``cv_options['scoring']='roc_auc'``),
            it can be one out of ``'balanced_accurracy'``, ``'f1'``, ``'f2'``,
            ``'J'``, ``'pseudo_f1'`` or ``'pseudo_f2'``.
            If multiple metrics are chosen (e.g. ``cv_options['scoring']=['roc_auc', 'mcc']``),
            it can be one of ``'balanced_accuracy'``, ``'f1'``, ``'f2'``, ``'J'``, ``'pseudo_f1'``
            or ``'pseudo_f2'`` (to perform the same threshold tuning method for all metrics)
            or a list of according length with elements out of ``['balanced_accuracy', 'f1',
            ...
            decision threshold by selecting the optimal threshold from the ROC curve by choosing
            the threshold with the maximum value of Youden's J statistic after performing
            hyperparameter optimization using ``'roc_auc'`` as scoring, while performing no
            threshold tuning after hyperparameter optimization using ``'mcc'`` as scoring.
            If ``'f1'`` or ``'f2'``, the optimal threshold will be selected
            from the Precision-Recall curve by choosing the threshold with the maximum
            value of the F-beta score with ``beta=1`` (F1 score) or ``beta=2`` (F2 score).
            If ``'balanced_accuracy'``, ``'J'``, ``'pseudo_f1'`` or ``'pseudo_f2'``, the optimal
            threshold will be selected from the ROC curve by choosing the threshold with the maximum
            value of the balanced accuracy, Youden's J statistic, pseudo-F1 or pseudo-F2.
            CAUTION: Please keep in mind that the scoring metric used during threshold tuning
            ...
            out of ``'balanced_accuracy'``, ``'pseudo_f1'``, ``'pseudo_f2'`` or ``'J'``.
        ...
        'tune_threshold' : bool, default=True
            If ``True``, perform decision threshold tuning on each outer training fold
            ...
            A list of decision thresholds is returned that maximize the scoring metric
